...
import {shiftingData} from '../../../api/constant';

export default function Shifting({navigation, route}) {
  const colors = useSelector(state => state.theme.theme);
  const title = route?.params?.item;

  const onPressContinue = () =>
    navigation.navigate(StackNav.ShiftingAddress, {
      item: title,
    });
